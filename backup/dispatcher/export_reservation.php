...
$validColumns = [
    'reservation_status', 'vehicle_type', 'reservation_date', 'location', 'duration',
    'time_departure', 'no_passengers', 'office_dept', 'email',
    'contact_no', 'service_type', 'purpose'
];
...
$sql = "SELECT $columnsToSelect FROM client_reservation WHERE YEAR(reservation_date) = ?";
...
$filenameBase = "client_reservation_report_" . $year;
...
if ($format == 'excel') {
    ...
    $pdf->SetKeywords('TCPDF, PDF, report, clientreservation');
    ...
    header("Content-Disposition: attachment;Filename=client_reservation_report_{$year}.doc");
    ...
    echo "<h3>Client Reservation Report for {$year}</h3>";
