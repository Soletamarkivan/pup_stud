...
function viewParts(vehicleID) {
    window.location.href = `/gearchanix/gearchanix-main/gearchanix/public/pages/supervisor/vehicles/vehicle-parts.html?vehicle_ID=${vehicleID}`;
}
    ...
    fetch(`editvehicle.php?id=${vehicle_ID}`)
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById("editOdometerLastPMS").value = data.odometer_last_pms || '';
...
document.getElementById("editVehicleForm").addEventListener("submit", function(e) {
    ...
    .then(response => response.text()) // Get response as text for better error handling
    .then(text => {
        try {
            const result = JSON.parse(text); // Parse JSON only if valid
            if (result.success) {
                loadVehicles(); 
            ...
            console.error("Response was not JSON:", text); // Log HTML response to console
            alert('Unexpected error: ' + text); // Display raw response in an alert
        }
    })
