                ...
                if ($vehicle_result->num_rows > 0) {
                    ...
                    if ($conn->query($insert_workorder_query)) {
                        ...
                        if ($insert_pms_stmt->execute()) {
                            // Delete matching row in service_reminder table
                            $delete_service_reminder_query = "DELETE FROM service_reminder WHERE service_task = ? AND plate_num = ?";
                            $delete_stmt = $conn->prepare($delete_service_reminder_query);
                            $delete_stmt->bind_param("ss", $service_task, $plate_num);

                            if ($delete_stmt->execute()) {
                                echo json_encode(['success' => true, 'message' => 'Approval updated, record inserted into PMS and Work Order successfully, and service reminder deleted.']);
                            } else {
                                echo json_encode(['success' => false, 'message' => 'Error deleting from service_reminder: ' . $delete_stmt->error]);
                            }
                        } else {
